...
def create_default_runtime(
    ...
    return Runtime(
        ...
        step_state_machine=step_state_machine or StepStateMachine(),
        attempt_state_machine=attempt_state_machine or AttemptStateMachine(),
...
def test_valid_transition_updates_status_and_history():
    ...
    runtime = create_default_runtime(state_machine=run_state_machine)
...
def test_valid_pending_to_running_step_transition():
    ...
    runtime = create_default_runtime(step_state_machine=step_state_machine)
...
def test_valid_pending_to_cancelled_step_transition():
    step_state_machine = StepStateMachine()
    runtime = create_default_runtime(step_state_machine=step_state_machine)
    step = Step(step_id="step-456", run_id="run-123")
...
def test_execute_step_successfully():
    ...
    runtime = create_default_runtime(executor=executor)
    step = Step(step_id="step-123", run_id="run-123")
...
def test_runtime_transitions_attempt():
    runtime = create_default_runtime(executor=MockExecutor())
